...
public class BootMenuScreen implements Screen {
    ...
    public void render() {
        ...
        int selection = bios.getBootMenuSelection();
        for (int i = 0; i < disks.size(); i++) {
            java.io.File disk = disks.get(i);
            String name = disk.getParentFile().getName() + ": " + disk.getName();
            java.awt.Color color = (i == selection) ? java.awt.Color.GREEN : java.awt.Color.WHITE;
        ...
        while (org.lwjgl.input.Keyboard.next()) {
            if (org.lwjgl.input.Keyboard.getEventKeyState()) {
                ...
                if (key == org.lwjgl.input.Keyboard.KEY_UP) {
                    if (selection > 0) bios.setBootMenuSelection(selection - 1);
                } else if (key == org.lwjgl.input.Keyboard.KEY_DOWN) {
                    if (selection < bios.getDetectedDisks().size() - 1) bios.setBootMenuSelection(selection + 1);
                } else if (key == org.lwjgl.input.Keyboard.KEY_RETURN && !bios.getDetectedDisks().isEmpty()) {
                    File disk = bios.getDetectedDisks().get(bios.getBootMenuSelection());
